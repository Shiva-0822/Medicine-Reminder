  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= days; day++) {
      ...
      if ((firstDay + day) % 7 === 0 || day === days) {
        // If this is the last week of the month and has fewer than 7 days,
        // pad the remaining slots with empty cells so each week has 7 items.
        if (day === days && week.length < 7) {
          for (let i = week.length; i < 7; i++) {
            week.push(
              <View key={`empty-end-${day}-${i}`} style={styles.calendarDay} />
            );
          }
        }
...
const styles = StyleSheet.create({
  ...
  calendarWeek: {
    flexDirection: "row",
    marginBottom: 5,
    // ensure rows don't wrap and stay a single line of 7 items
    flexWrap: "nowrap",
  },
  calendarDay: {
    width: "14.2857%",
